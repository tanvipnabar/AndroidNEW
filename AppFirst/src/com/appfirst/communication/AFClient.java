...
public class AFClient {
	 ...
	 * @param url
	 *            the public api address of the query
	 * @return a {@link UserProfile} object.
	 */
	
	public List<UserProfile> getUserList(String url) {
		JSONArray jsonArray = null;
		HttpGet getRequest = new HttpGet(url);
		jsonArray = makeJsonArrayRequest(getRequest);
		
		return Helper.convertUserList(jsonArray);
	}
	...
	public UserProfile getUserProfile(String url) {
		JSONObject jsonObject = null;
		HttpGet getRequest = new HttpGet(url);
		jsonObject = makeJsonObjectRequest(getRequest);
